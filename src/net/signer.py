...
def hash_message(account: int, to: int, selector: int, calldata: List[int], nonce: int) -> int:
...
class Signer(Client):
    def __init__(self, address: AddressRepresentation, key_pair: KeyPair, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.address = parse_address(address)
        self.key_pair = key_pair

    async def add_transaction(
        self, tx: InvokeFunction, token: Optional[str] = None,
    ) -> Dict[str, int]:
