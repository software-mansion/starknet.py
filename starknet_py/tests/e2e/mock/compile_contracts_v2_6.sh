...
CONTRACTS_DIRECTORY="$MOCK_DIRECTORY"/contracts_v2_6
CONTRACTS_COMPILED_DIRECTORY="$MOCK_DIRECTORY"/contracts_compiled_v2_6
...
for contract in "$CONTRACTS_DIRECTORY"/*.cairo; do
    ...
    cargo run --bin starknet-sierra-compile --manifest-path $MANIFEST_PATH -- --add-pythonic-hints $contract_json $contract_casm &> "$tmp"
