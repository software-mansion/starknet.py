...
async def hello_starknet_deploy_transaction_address(
    account: Account, v1_hello_starknet_class_hash
) -> int:
    deployer = Deployer()
...
@pytest_asyncio.fixture(scope="package")
async def block_with_declare_v2_number(v1_hello_starknet_tx_hash: int, client) -> int:
    """
    Returns number of the block with DeclareV2 transaction
    """
    declare_v2_receipt = await client.get_transaction_receipt(v1_hello_starknet_tx_hash)
