...
async def test_using_account_client(run_devnet):
    ...
    acc_client = await DevnetClientFactory(run_devnet).make_devnet_client()  # Remove from documentation
    ...
    await (await map_contract.functions["put"].invoke(k, v, max_fee=0)).wait_for_acceptance()
