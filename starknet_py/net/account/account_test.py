...
async def test_get_balance_default_token_address(net):
    ...
    assert call.to_addr == parse_address(FEE_CONTRACT_ADDRESS_ETH)
...
async def test_account_get_balance_strk(account, map_contract):
    balance = await account.get_balance(token_address=FEE_CONTRACT_ADDRESS_STRK)
    block = await account.client.get_block(block_number="latest")
    ...
    new_balance = await account.get_balance(token_address=FEE_CONTRACT_ADDRESS_STRK)
    old_balance = await account.get_balance(
        token_address=FEE_CONTRACT_ADDRESS_STRK, block_number=block.block_number
    )
